...
export function AppProvider({ children }) {
  ...
  const isDriveAuthError = useCallback((error) => {
    ...
    return (
      message.includes('Drive request failed: 401') ||
      message.includes('Drive request failed: 403') ||
      message.includes('Drive auth expired') ||
      message.includes('No Google access token found') ||
      message.includes('Google Drive session expired')
    )
  }, [])
